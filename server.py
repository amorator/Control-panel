...
from flask import render_template, url_for, request, send_from_directory, redirect, session, abort
...
def users_add():
    try:
        ...
        app._sql.user_add([login, name, app.hash(request.form.get('password')), int(request.form.get('enabled') != None), request.form.get('permission')])
...
def users_edit(id):
    try:
        ...
        app._sql.user_edit([login, name, int(request.form.get('enabled') != None), request.form.get('permission'), id])
    except Exception as e:
        app.flash_error(e)
    finally:
        return redirect(url_for('users'))
