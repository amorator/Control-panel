...
class SQL(Config):
    ...
    def with_conn(func):
        def _with_conn(self, command, args=[]):
            self.conn = mysql.connect(host=self.config['db']['host'], user=self.config['db']['user'], password=self.config['db']['password'], database=self.config['db']['name'])
...
class SQLUtils(SQL):
	...
	PRIMARY KEY(id)
        ...
        self.execute_non_query(f"INSERT IGNORE INTO {self.config['db']['prefix']}_user VALUES (%s, %s, %s, %s, %s, %s);", [1, "admin", self.config['admin']['name'], self.config['admin']['password'], 1, 'z,' * (int(self.config['db']['permission_length']) - 1) + 'z'])
    ...
    def user_add(self, args):
        self.execute_non_query(f"INSERT INTO {self.config['db']['prefix']}_user (login, name, password, enabled, permission) VALUES (%s, %s, %s, %s, %s, %s);", args)

    def user_edit(self, args):
        self.execute_non_query(f"UPDATE {self.config['db']['prefix']}_user SET login = %s, name = %s, enabled = %s, permission = %s WHERE id = %s;", args)
